...
# --- WeatherStack API Population Script (Standard) 🌦️ ---
# This script creates 30 properties across the USA using the GraphQL API.
# Includes a 3-second delay between requests to support Free/Basic API plans.
...
printf "%b🚀 Starting USA Property Population (Standard - 3s delay)...%b\n" "${BLUE}" "${NC}"
printf "%s\n" "------------------------------------------"
...
EOF
  ...
  # 3-second delay to support free tier
  sleep 3
done
